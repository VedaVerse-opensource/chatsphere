import { IoPersonCircleOutline, IoPencil, IoCheckmark } from "react-icons/io5";
...
export const UserBubble = ({ text, onEdit, onSave }) => {
  ...
  return (
    <div
      className='flex justify-end w-full mb-3 sm:mb-4'
    ...
    >
      <div className='flex items-end space-x-2 max-w-[85%] relative'>
        {isHovered && !isEditing && (
          <IoPencil
            size={20}
            className='absolute left-[-20px] top-1/2 transform -translate-y-1/2 cursor-pointer text-gray-500 hover:text-gray-700'
        ...
        {isEditing ? (
          <div className='bg-[#dddddd] dark:bg-dark-surface text-gray-800 dark:text-dark-text rounded-2xl py-2 sm:py-3 px-3 sm:px-4 border dark:border-gray-600 w-full'>
            ...
            <IoCheckmark
              size={20}
              className='absolute right-0 top-1/2 transform -translate-y-1/2 cursor-pointer text-gray-500 hover:text-gray-700'
              onClick={handleSave}
            />
          </div>
        ) : (
